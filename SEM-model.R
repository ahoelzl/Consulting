...
get.sem.coeffs <- function() {
  ...
  for( i in 1:length(emos)) {
    emo <- emos[i]
    
    results <- colnames(data.o[,(grep(emo,names(data.o)))])
    ...
    data.partial1 <- apply(data.partial1, FUN=as.numeric, MARGIN=2)
    data.partial2 <- apply(data.partial2, FUN=as.numeric, MARGIN=2)
    data.partial3 <- apply(data.partial3, FUN=as.numeric, MARGIN=2)
    data.partial4 <- apply(data.partial4, FUN=as.numeric, MARGIN=2)
    dp <- na.omit(data.partial)

    
    model <- paste0("emo1 =~ ", write.as.sum(emo1)," \n ",
                    "emo2 =~ ", write.as.sum(emo2)," \n ",
                    "emo3 =~ ", write.as.sum(emo3)," \n ",
                    "emo4 =~ ", write.as.sum(emo4)," \n ",
                    "emo5 =~ ", write.as.sum(emo5), " \n ",
                    "gma_jz6 ~ emo1 + gma_jz5", " \n ",
                    "gma_jz7 ~ emo2 + gma_jz6" ,"  \n ",
                    "gma_jz8 ~ emo3 + gma_jz7" , "\n ",
                    "gma_jz9 ~ emo4 + gma_jz8 " )

    #fit <- sem(model.1, data = na.omit(data.partial), mimic="MPLUS")
    
    
    
    fit <- sem(model, data = na.omit(data.partial), mimic="MPLUS")
    
    fits[emo] <- fit
    

    coef1 <- coef(fit)["gma_jz6~emo1"]
    coef2 <- coef(fit)["gma_jz7~emo2"]
    coef3 <- coef(fit)["gma_jz8~emo3"]
    coef4 <- coef(fit)["gma_jz9~emo4"]
    
    coefs.matrix.sem[i,1] <- coef1
    coefs.matrix.sem[i,2] <- coef2
    coefs.matrix.sem[i,3] <- coef3
    coefs.matrix.sem[i,4] <- coef4

    
  }
  fits <<- fits
  theoretical.values <<-  coefs.matrix.sem

  
  
  colnames(coefs.matrix.sem) <- c("T1", "T2","T3","T4")

  rownames(coefs.matrix.sem) <- emos
